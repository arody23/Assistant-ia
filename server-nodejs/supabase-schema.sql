...
-- ============================================================================
-- MIGRATION v7 — auth dashboard admin (RLS)
-- Remplace les policies anon ouvertes par accès admin authentifié uniquement.
-- Le bot Node.js (service_role) et le chatbot public (/api/webchat) ne sont pas impactés.
-- ============================================================================
...
-- Lecture du profil pour vérifier le rôle après login
...
-- Storage — lecture publique conservée, écriture admin uniquement
drop policy if exists "Anon insert ambassador-media" on storage.objects;
drop policy if exists "Anon update ambassador-media" on storage.objects;
drop policy if exists "Anon delete ambassador-media" on storage.objects;
