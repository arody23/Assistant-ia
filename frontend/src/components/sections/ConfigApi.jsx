...
import { Card, Input, Select, RedButton, Field } from "@/components/Primitives";
import { Save, Cpu, Mic } from "lucide-react";

const MODELS = [
  { v: "llama-3.1-8b-instant", l: "Llama 3.1 8B Instant (rapide · défaut)" },
...
export default function ConfigApi({ config, updateConfig }) {
  ...
  const save = () => updateConfig({
    model: form.model, fallback_model: form.fallback_model, whisper_model: form.whisper_model,
    max_tokens: Number(form.max_tokens), temperature: Number(form.temperature),
    delay_ms: Number(form.delay_ms), memory_msgs: Number(form.memory_msgs),
  });

  return (
    <div className="space-y-4 sm:space-y-6">
      <Card title="Modèles Groq" subtitle="Sélection du moteur IA" testid="card-groq-models"
        action={<RedButton onClick={save} testid="config-save-btn"><Save size={12} className="mr-1" /> Sauver</RedButton>}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
          <Field label="Modèle principal" hint="Réponses rapides aux clients">
          ...
          <Field label="Modèle de fallback" hint="Si le principal échoue / cas complexes">
      ...
      <Card title="Limites & Quotas" subtitle="Contrôle conversationnel" testid="card-limits"
        action={<RedButton onClick={save} testid="limits-save-btn">Sauver</RedButton>}>
      ...
      <Card title="Endpoints" testid="card-endpoints">
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3 text-[11px] text-[var(--vsm-grey)]">
          <span className="flex items-center gap-1.5"><Cpu size={14} className="text-[var(--vsm-red)]" />Backend Node.js : <span className="font-mono text-[var(--vsm-red)]">/app/server-nodejs/</span></span>
          <span className="flex items-center gap-1.5"><Mic size={14} className="text-[var(--vsm-red)]" />Whisper actif</span>
        </div>
      </Card>
    </div>
  );
